...
public class SearchAutoCompleteServiceImpl implements SearchAutoCompleteService {
	...
	public List<Map<String, String>> getCategoryList(String q) {
		List<Map<String, String>> result = new ArrayList<Map<String, String>>();
		List<Category> start = Category.find("level = 1 and name like :start order by name asc").bind("start", q+"%").fetch(6);
		for (Category c : start) {
    		result.add(c.toTokenMap());
    	}
		int extra = result.size() < 6 ? 6 - result.size() : 6;
		if (extra > 0) {
			List<Category> contain = Category.find("level = 1 and name like :contain and name not like :start order by name asc").bind("contain", "%"+q+"%").bind("start", q+"%").fetch(extra);
